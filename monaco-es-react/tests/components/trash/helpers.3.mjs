...
import {TemplateSpans} from './templates.mjs';
...
export function html(strings, ...values) {
  ...
  const interpolate = strings => {
    ...
    if (length) {
      ...
      const marks = spans.marks; // [strings, ... spans.marks];
      ...
      const prerendered = render([strings, ... spans.marks]);
      ...
      if (attributeSpans || literalSpans) {
        ...
        if (literalSpans) {
          ...
          while ((node = nodeIterator.nextNode())) {
            ...
            // if (asymmetric) {
            //   if (nodeName === 'template') console.warn('LiteralSpans - Template: %o', element);
            //   // TODO: Split Text
            // }
            // const children = element.childNodes; // [... element.childNodes];
...
export function Throw(error) {
  ...
  throw !error ? Throw.sloppy || (Throw.sloppy = Error('Ouch: sloppy code!'))
    : (!('message' in error && 'type' in error) && Error(error)) ||
    error;
}
