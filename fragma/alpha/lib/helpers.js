...
import {polyfills, NodeTypes} from './polyfills.js';
import {
  TemplateSpans,
  TemplateSpan,
...
/// CACHES ///
const defaultCaches = (async () => {
  try {
    await self.caches.open('default');
  } catch (exception) {
...
export function html(strings, ...values) {
  ...
  const AttributeSpans = /^((?:[^>]+|[^]*<[a-z][^>]+)\s+)([a-z](?:[a-z-]+[a-z])*)(\s*=)(["']|)$/g;
  // const AttributeSpans = /^((?:(?:[^>]*\n?)+|(?:.*\n?)*<[a-z][^>]+)\s+)([a-z](?:[a-z-]+[a-z])*)(\s*=)(["']|)$/gm;
